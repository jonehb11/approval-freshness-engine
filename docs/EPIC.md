...
| **Availability failure** | Stays natively blocked on the static ruleset (per-PR reaper + circuit breaker handle the deterministic layer); a fresh human approval on the current head is always available to unblock via the redundant fresh-approval echo, engine up or down. No merge freeze, no fail-open. Manual kill switch (human un-enrollment) also available. |
...
Enrolled-repo ruleset requires `approval-freshness/evaluated`, with `integration_id` pinned to the engine's GitHub App so no other workflow can satisfy it (closing the exact hole a same-named check from `github-actions` would otherwise open). Engine sets it `pending`/`in_progress` on every synchronize, then `success` only on a preserve decision. Dismiss decisions leave it non-green (merge blocked pending fresh human approval — identical to today's outcome). If the engine is down — briefly or for a long stretch — a fresh push simply waits on the required check, exactly as it would for any other down CI job; nothing about the ruleset changes. The unblock path is always the same: a fresh human approval on the current head SHA, echoed to a green check either by the engine or by a redundant GitHub-Actions-hosted fallback authenticated as the same App. The required check gates *individual freshly-pushed PRs* until that happens — it never freezes the org on engine health, and it never triggers a ruleset mutation.
