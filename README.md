...
| **Resolving a merge conflict by hand** — evaluated on content (the delta is re-based onto the base branch so anything the resolution *discarded* is visible). In practice these almost always dismiss, because choosing a winner between two colliding changes is a real decision | `model_high_impact` / `corroboration_gate_failed`; `merge_conflict_resolution` only when the resolution cannot be inspected |
